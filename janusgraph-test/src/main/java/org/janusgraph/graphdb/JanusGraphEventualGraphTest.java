...
//        System.out.println("Time: " + (System.currentTimeMillis()-start));
    ...
    public void testConsistencyModifier() throws InterruptedException {
        ...
        Edge e = getOnlyElement((Iterable<JanusGraphEdge>) v.query().direction(OUT).labels("es").edges());
        assertEquals(wintx,e.<Integer>value("sig").intValue());
        assertNotEquals(rs[6].longId(),getId(e));

        e = getOnlyElement((Iterable<JanusGraphEdge>) v.query().direction(OUT).labels("o2o").edges());
        assertEquals(wintx,e.<Integer>value("sig").intValue());
        assertEquals(rs[7].longId(), getId(e));
        e = getOnlyElement((Iterable<JanusGraphEdge>) v.query().direction(OUT).labels("o2m").edges());
        assertEquals(wintx,e.<Integer>value("sig").intValue());
        assertNotEquals(rs[8].longId(),getId(e));
        e = getOnlyElement((Iterable<JanusGraphEdge>) v.query().direction(OUT).labels("em").edges());
        assertEquals(wintx,e.<Integer>value("sig").intValue());
        assertEquals(rs[4].longId(), getId(e));
        for (Edge ee : ((Iterable<JanusGraphEdge>) v.query().direction(OUT).labels("emf").edges())) {
    ...
    private void processTx(JanusGraphTransaction tx, int txid, long vid, long uid) {
        ...
        for (String label : new String[]{"em","emf"}) {
            e = getOnlyElement((Iterable<JanusGraphEdge>) v.query().direction(OUT).labels(label).edges());
            assertEquals(1,e.<Integer>value("sig").intValue());
            sign((JanusGraphEdge)e,txid);
        }
    }
