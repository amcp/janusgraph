    ...
    private static class FixedRelationshipMaker implements Runnable {
        ...
        public void run() {
            while (true) {
                ...
                Iterable<JanusGraphEdge> edges = source.query().direction(Direction.OUT).labels(elabel).edges();
                for (Edge r : edges) {
    ...
    private static class SimpleReader extends BarrierRunnable {
        ...
        protected void doRun() throws Exception {
            ...
            for (int i = 0; i < nodeTraversalCount; i++) {
                ...
                Iterable<JanusGraphEdge> edges =  v.query().direction(Direction.OUT).labels(label2Traverse).edges();
                for (JanusGraphEdge r : edges) {
