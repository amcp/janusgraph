...
public abstract class JanusGraphSpeedTest extends SpeedTestSupport {

    private static final Logger log = LoggerFactory.getLogger(JanusGraphSpeedTest.class);

    @Rule
    public TestRule benchmark = JUnitBenchmarkProvider.get();

    protected JanusGraphSpeedTest(WriteConfiguration conf) throws BackendException {
        super(conf);
    }
    ...
    void testVertexUidLookup() throws Exception {
        super.sequentialUidTask((JanusGraphTransaction tx, JanusGraphVertex vertex) -> {
    ...
    void testVertexCentricIndexQuery() {
        ...
        super.supernodeTask((JanusGraphVertex v, String indexLabel, String indexPK) -> {

            int c = v.outE(indexLabel)
    ...
    void testLabeledEdgeTraversal() {
        ...
        super.supernodeTask((JanusGraphVertex v, String indexLabel, String indexPK) -> {
            int start = 100 * i++;
            int end = start + 99;
            int c = v.outE(indexLabel)[start..end].inV().outE(indexLabel).inV().outE(indexLabel).count();
            assertTrue(0 < c);
        });
    }
    ...
    void testEdgeTraversalUsingVertexCentricIndex() {
        super.supernodeTask((JanusGraphVertex v, String label, String pkey) -> {
            int c = v.outE(label)
    ...
    void testLimitedGlobalEdgePropertyQuery() {
        super.standardIndexEdgeTask((JanusGraphTransaction tx, String indexedPropName, Object indexedPropVal) -> {
    ...
    void testLimitedGlobalVertexPropertyQuery() {
        super.standardIndexVertexTask((JanusGraphTransaction tx, String indexedPropName, Object indexedPropVal) -> {
    ...
    public void testSingleVertexMultiProperty() {
        super.sequentialUidTask(50, (JanusGraphTransaction tx, JanusGraphVertex v) -> {
            int c = 0;
            ...
            Object k = ElementHelper.getPropertyKeys(v) - 1;
    ...
    public void testSingleVertexProperty() {
        super.sequentialUidTask(50, (tx, v) -> {
    ...
    */
    ...
    void testZVertexPropertyModification() {
        ...
        int visited = 0;
        int n = 314159;
        super.sequentialUidTask(null, (tx, vertex) -> {
            visited++;
            for (String p : ElementHelper.getPropertyKeys(v)) {
    ...
    void testZEdgeAddition() {
        ...
        super.sequentialUidTask(null, (JanusGraphTransaction tx, JanusGraphVertex vertex) -> {
            if (-1 != last && last != (Long) vertex.id()) {
...
//    }
    ...
    private void multiVertexQueryTask(JanusGraphTransaction tx, JanusGraphVertex[] vbuf, int vcount) {
        if (vcount != vbuf.length) {
        	JanusGraphVertex[] newbuf = new JanusGraphVertex[vcount];
        ...
        for (int i = 0; i < schema.getEdgeLabels(); i++) {
            Map<JanusGraphVertex, Iterable<JanusGraphEdge>> m = tx.multiQuery(vbuf).labels(schema.getEdgeLabelName(i)).edges();
    ...
    private void singleVertexQueryTask(JanusGraphTransaction tx, JanusGraphVertex v) {
        int n = 0;
        for (int i = 0; i < schema.getEdgeLabels(); i++) {
            for (JanusGraphEdge iter : (Iterable<JanusGraphEdge>) v.query().labels(schema.getEdgeLabelName(i)).edges()) {
                n++;
            }
        }
        assertTrue(0 < n);
